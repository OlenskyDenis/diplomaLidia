...
class VoyagerController extends Controller
{
    ...
    public function predict(Request $request)
    {
        ...
        $csv = \League\Csv\Writer::createFromFileObject(new \SplFileObject('storage/assets/py/researchesPredict.csv', 'w'));
        ...
        $reqOnly = implode(",", array_values($request->only([
            'lkp',
            'bsk5',
            'hsk',
            'soluble_oxygen',
        ...
        if ($request->has("roundworms"))
            $reqOnly .= ",1";
        else
            $reqOnly .= ",0";
        $output = explode("\n", shell_exec($command . $reqOnly));
